...
import SingleHabit from "./HabitCard"
...
export default function Habits(){
	...
	useEffect(()=>{
		const promisse = axios.get("https://mock-api.bootcamp.respondeai.com.br/api/v2/trackit/habits", {headers: {"Authorization": `Bearer ${data.token}`}})
		promisse.then((promisseData)=>{
			setData({...data, allHabits: [...promisseData.data] })
		})
		promisse.catch(()=>{
	...
	function postHabit(event){
		...
		const promisse = axios.post("https://mock-api.bootcamp.respondeai.com.br/api/v2/trackit/habits", newHabit, {headers: {"Authorization": `Bearer ${data.token}`}} )
		promisse.then(()=>{
			setDisabled(false)
			displayNewHabit()
